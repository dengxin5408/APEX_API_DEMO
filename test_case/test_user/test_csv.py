...
def test_chaxun(pub_dic,phone,assertion): # get请求query数据
    ...
    feature = "查询一级分类"  # allure报告中一级分类
    story = '查询二级分类'  # allure报告中二级分类
    title = "查询用例"  # allure报告中用例名字
    # 将公共方法上传,保持已登录
    d = {"phone":'phone'}
    # 将params存入变量d中
    r = request_tool.get(url="http://api.yansl.com:8084/cst/getCustomer",params=d,headers=headers,method=method,feature=feature,story=story,title=title)
